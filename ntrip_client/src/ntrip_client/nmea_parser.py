...
class NMEAParser:
    ...
    def is_valid_sentence(self, sentence):
        ...
        if len(sentence) > _NMEA_MAX_LENGTH:
            self._logwarn(
                'Received invalid NMEA sentence. Max length is {}, '
                'but sentence was {} bytes'.format(
                    _NMEA_MAX_LENGTH, len(sentence)))
            self._logwarn('Sentence: {}'.format(sentence))
            return False
        if len(sentence) < _NMEA_MIN_LENGTH:
            self._logwarn(
                'Received invalid NMEA sentence. We need at least {} '
                'bytes to parse but got {} bytes'.format(
                    _NMEA_MIN_LENGTH, len(sentence)))
            self._logwarn('Sentence: {}'.format(sentence))
            return False
        if sentence[0] != '$' and sentence[0] != '!':
            self._logwarn(
                'Received invalid NMEA sentence. Sentence should begin '
                'with "$" or "!", but instead begins with {}'.format(
                    sentence[0]))
            self._logwarn('Sentence: {}'.format(sentence))
            return False
        if sentence[-2:] != '\r\n':
            self._logwarn(
                'Received invalid NMEA sentence. Sentence should end '
        ...
        if _NMEA_CHECKSUM_SEPERATOR not in sentence:
            self._logwarn(
                'Received invalid NMEA sentence. Sentence should have a '
                '"{}" character to seperate the checksum, but we could '
                'not find it.'.format(_NMEA_CHECKSUM_SEPERATOR))
            self._logwarn('Sentence: {}'.format(sentence))
            return False
        ...
        if expected_checksum != calculated_checksum:
            ...
            self._logwarn(
                'Calculated Checksum: 0x{:X}'.format(
                    calculated_checksum))
            return False
