...
Read-only query identifiers are `APPNG`, `APVER`, `APSER`, `APSTA`, `APPID`, `APFSN`, `APFHW`, and `APIHW`, with no arguments; `APECH` accepts printable echo text. `APCFG`/`APVEH` permit `r`/`R` operations with nonempty parameter names. The exact keyless discovery forms `APCFG,r`, `APCFG,R`, and `APVEH,R` are also permitted; trailing commas remain invalid. Unsupported firmware queries may still return APERR. Configuration replies are bounded to 4096 bytes including framing; oversized replies fail explicitly. The [snapshot exporter](device_configuration_export.md) uses these read-only queries.
